...
namespace Etoile
{
	...
	public:
		...
		{ 
			...
			for(int a=0; a < sqrt_n_samples; a++) { 
				for(int b=0; b < sqrt_n_samples; b++) { 
					...
					samples[i].m_sph = Vec3d(theta,phi,1.0); 
					...
					for(int l=0; l < n_bands; ++l) { 
						for(int m=-l; m<=l; ++m) { 
							int index = l*(l+1)+m; 
							samples[i].p_coeff[index] = SH(l,m,theta,phi); 
						} 
					} 
					++i; 
				} 
		...
		{ 
			...
			{ 
				double theta = samples[i].m_sph.x(); 
				double phi   = samples[i].m_sph.y(); 
				for(int n=0; n < n_coeff; ++n) 
				{ 
					result[n] += fn(theta, phi) * samples[i].p_coeff[n]; 
				} 
			} 
