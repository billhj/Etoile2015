...
#define THRESHOLD_ROTATION 0.01
	...
	public:
		...
		void setAxisAngle(const Vec3<T> & axis, T  angle)
		{
			...
			{
				// Null rotation
				m_q[0] = 0.0;      m_q[1] = 0.0;      m_q[2] = 0.0;      m_q[3] = 1.0;
			}
			else
			{
				const T  sin_half_angle = sin(angle / 2.0f);
				m_q[0] = sin_half_angle * axis[0]/norm;
				m_q[1] = sin_half_angle * axis[1]/norm;
				m_q[2] = sin_half_angle * axis[2]/norm;
				m_q[3] = cos(angle / 2.0f);
			}
		}
		...
		*/
		...
		{ 
			return Quaternion(m_q[0]*scale,m_q[1]*scale,m_q[2]*scale,m_q[3]*scale); 
		}
		...
		{
			return Quaternion(a.m_q[3]*b.m_q[0] + b.m_q[3]*a.m_q[0] + a.m_q[1]*b.m_q[2] - a.m_q[2]*b.m_q[1],
				a.m_q[3]*b.m_q[1] + b.m_q[3]*a.m_q[1] + a.m_q[2]*b.m_q[0] - a.m_q[0]*b.m_q[2],
				a.m_q[3]*b.m_q[2] + b.m_q[3]*a.m_q[2] + a.m_q[0]*b.m_q[1] - a.m_q[1]*b.m_q[0],
				a.m_q[3]*b.m_q[3] - b.m_q[0]*a.m_q[0] - a.m_q[1]*b.m_q[1] - a.m_q[2]*b.m_q[2]);
		}
		...
		{
			if(m_q[3]==q.m_q[3] && m_q[2]==q.m_q[2] &&m_q[1]==q.m_q[1] &&m_q[0]==q.m_q[0])
				return true;
			return false;
		}
		...
		Vec3<T>  axis() const
		{
			Vec3<T>  res = Vec3<T>(m_q[0], m_q[1], m_q[2]);
		...
		T  angle() const
		{
			const T angle = (T)2.0 * acos(m_q[3]);
			return (T)(angle <= M_PI) ? angle : 2.0*M_PI - angle;
		}

		void getAxisAngle(Vec3<T> & axis, T & angle) const
		{
			angle = 2.0*acos(m_q[3]);
			axis = Vec3<T>(m_q[0], m_q[1], m_q[2]);
		...
		void invert() { m_q[0] = -m_q[0]; m_q[1] = -m_q[1]; m_q[2] = -m_q[2]; }

		Quaternion inverse() const 
		{ 
			T  scalar = 1 / ( m_q[0] * m_q[0] + m_q[1] * m_q[1] + m_q[2] * m_q[2] + m_q[3] * m_q[3]);
			return conjugate() * scalar; 
		}


		void negate() { invert(); m_q[3] = -m_q[3]; }
		...
		T normalize()
		{
			const T norm = sqrt(m_q[0]*m_q[0] + m_q[1]*m_q[1] + m_q[2]*m_q[2] + m_q[3]*m_q[3]);
			for (int i=0; i<4; ++i)
				m_q[i] /= norm;
			return norm;
		}
		...
		{
			const T  q00 = 2.0 * m_q[0] * m_q[0];
			const T  q11 = 2.0 * m_q[1] * m_q[1];
			const T  q22 = 2.0 * m_q[2] * m_q[2];

			const T  q01 = 2.0 * m_q[0] * m_q[1];
			const T  q02 = 2.0 * m_q[0] * m_q[2];
			const T  q03 = 2.0 * m_q[0] * m_q[3];

			const T  q12 = 2.0 * m_q[1] * m_q[2];
		...
		Quaternion log()
		{
			T len = sqrt(m_q[0]*m_q[0] + m_q[1]*m_q[1] + m_q[2]*m_q[2]);

			if (len < 1E-6)
				return Quaternion(m_q[0], m_q[1], m_q[2], 0.0);
			else
			{
				T coef = acos(m_q[3]) / len;
		...
		Quaternion exp()
		{
			T theta = sqrt(m_q[0]*m_q[0] + m_q[1]*m_q[1] + m_q[2]*m_q[2]);
		...
		void fromEulerXYZ(T roll, T pitch, T yaw)
		{
			...
			m_q[0] = sinr * cosp * cosy - cosr * sinp * siny;
			m_q[1] = cosr * sinp * cosy + sinr * cosp * siny;
			m_q[2] = cosr * cosp * siny - sinr * sinp * cosy;
			m_q[3] = cosr * cosp * cosy + sinr * sinp * siny;
		...
		{
			//atan2
			T roll = atan2(2 * (m_q[3]* m_q[0] + m_q[1]* m_q[2]),   1 - 2*( m_q[0] * m_q[0] + m_q[1] * m_q[1]));
			T v = m_q[3] * m_q[1] - m_q[2] * m_q[0];
			if(v > 0.5) v = 0.5;
			if(v < -0.5) v = -0.5;
			T pitch = asin(2* ( v ));
			T yaw = atan2( 2 * ( m_q[3] * m_q[2] +  m_q[0] * m_q[1]), 1 - 2*( m_q[1] * m_q[1] + m_q[2] * m_q[2]));

			if(v == 0.5)
			{
				roll = 2 * atan2(m_q[0], m_q[3]);
				pitch = M_PI * 0.5;
				yaw = 0;
			}
			else if(v == - 0.5)
		...
		{
			m_q[3] = -cos( (a - c) * 0.5) * sin(b * 0.5);
			m_q[0] = -sin( (a - c) * 0.5) * sin(b * 0.5);
			m_q[1] = -sin( (a + c) * 0.5) * cos(b * 0.5);
			m_q[2] =  sin( (a + c) * 0.5) * cos(b * 0.5);
		}

		Vec3<T>  getEulerAngleZYZ()const
		{
			T a = atan2( (m_q[3] *m_q[1] + m_q[0]*m_q[2]), (m_q[0]*m_q[1] - m_q[3]*m_q[2]));
			T b = acos(-m_q[3] * m_q[3] - m_q[0] *m_q[0] + m_q[1] * m_q[1] + m_q[2] *m_q[2]);
			T c = - atan2( (m_q[3] *m_q[1] - m_q[0]*m_q[2]), (m_q[0]*m_q[1] + m_q[3]*m_q[2]));
			return Vec3<T>(a,b,c);
		}
