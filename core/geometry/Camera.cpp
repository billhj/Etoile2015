...
namespace Etoile
{
	...
	Camera::Camera(const Vec3f& position, const Quaternionf& orientation, const std::string& name) : Entity(name, SceneManager::getInstance()->getCurrentScene())
	{
		reset();
	...
	Camera::Camera(const Vec3f& target, const Vec3f& up, const Vec3f& position, const std::string& name): Entity(name, SceneManager::getInstance()->getCurrentScene())
	{
		reset();
		this->setupCameraOrientation(target, up, position);
	}

	Camera::Camera(const std::string& name): Entity(name, SceneManager::getInstance()->getCurrentScene())
	{
		reset();
		m_pivot = getViewDirection() * 2 + p_transform->getPosition();
	}
