...
namespace Etoile
{
	...
	bool IKTree::loadFromFile(const std::string& fileName)
	{
		std::fstream in(fileName.c_str(), std::ios_base::in );
	...
	void IKTree::read(std::istream& in)
	{
		std::string line;
		int lineNb = 0;
		while( in && !in.eof() )
		{
			...
			try
			{
				...
				}else
				{
					...
					while(!stream.fail())
					{
						m_joint_end_effector_index.push_back(endid);
						stream >> endid;
					}
					std::getline(in,line);
					std::stringstream stream(line);
					...
					while(!stream.fail())
					{
						m_joint_identifier.push_back(ids);
						stream >> ids;
					}
					for(int i = 0 ; i < m_joint_end_effector_index.size(); ++i)
					{
						m_dim_end_effector_index.push_back(m_joints[m_joint_end_effector_index[i]]->m_dims.back().m_idx);
					}
					for(int i = 0 ; i < m_joint_identifier.size(); ++i)
					{
						for(int j = 0; j < m_joints[i]->m_dims.size(); ++j)
						{
							m_dim_identifier.push_back(m_joint_identifier[i]);
						}
					}
				}
			}catch(std::exception& e)
