...
namespace Etoile
{
	...
	class JacobianDLSSolver : public IKSolver
	{
		float m_dampling;
	public:
		JacobianDLSSolver(IKChain* chain, int maxTries = 50, float targetThreshold = 0.005, float stepweight= 0.5, float dampling = 1):IKSolver(chain, maxTries, targetThreshold, stepweight),
		m_dampling(dampling){}
