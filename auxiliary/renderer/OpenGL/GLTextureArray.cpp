...
namespace Etoile
{
	...
	bool GLTextureArray::create( int iWidth, int iHeight, int iLayer, GLenum internalFormat, GLenum pixeldataformat, GLenum type, float* data, bool bCreateMipMap)
	{
		...
		printOpenGLError();
		glBindTexture(GL_TEXTURE_2D_ARRAY_EXT, m_id);
		printOpenGLError();
		...
		printOpenGLError();
		glTexImage3D(GL_TEXTURE_2D_ARRAY_EXT,0, m_internalFormat, m_width, m_height,m_layer, 0, pixeldataformat, type, data);
		printOpenGLError();
