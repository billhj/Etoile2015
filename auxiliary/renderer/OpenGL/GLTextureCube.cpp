...
namespace Etoile
{
	...
	*/
	bool GLTextureCube::create(  int iWidth, int iHeight, int iLayer, GLenum internalFormat, GLenum pixeldataformat, GLenum type, float* data, bool bCreateMipMap)
	{
		...
		printOpenGLError();
		glTexImage3D(GL_TEXTURE_CUBE_MAP,0, m_internalFormat, m_width, m_height,m_layer, 0, pixeldataformat, type, data);
		printOpenGLError();
	...
	//img.save("img.jpg");
	m_height = img.height()/4.0;
	m_width = img.width()/3.0;
	QPixmap qpmp;
	qpmp = QPixmap::fromImage(img);

	QPixmap top , bottom,  left, right, back , front;
	top = qpmp.copy(m_width,0,m_width,m_height);
	bottom =  qpmp.copy(m_width, m_height*2 ,m_width,m_height);
	left = qpmp.copy(0, m_height ,m_width,m_height);
	right =  qpmp.copy( m_width * 2, m_height ,m_width,m_height);
	front =  qpmp.copy( m_width, m_height,m_width,m_height);
	back = qpmp.copy(m_width, m_height*3,m_width,m_height);
	...
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X_EXT, 0, GL_RGBA32F_ARB, m_width, m_height,  0, GL_RGBA,  GL_UNSIGNED_BYTE, leftI.bits());

	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y_EXT, 0, GL_RGBA32F_ARB, m_width, m_height,  0, GL_RGBA,  GL_UNSIGNED_BYTE,topI.bits() );

	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_EXT, 0, GL_RGBA32F_ARB, m_width, m_height,  0, GL_RGBA,  GL_UNSIGNED_BYTE,bottomI.bits() );
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z_EXT, 0, GL_RGBA32F_ARB, m_width, m_height,  0, GL_RGBA,  GL_UNSIGNED_BYTE, frontI.bits());
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_EXT, 0, GL_RGBA32F_ARB, m_width, m_height,  0, GL_RGBA,  GL_UNSIGNED_BYTE,  backI.bits());
	printOpenGLError();
	return true;
