...
namespace Etoile
{
	...
	public:
		...
		bool reInit(Renderer* renderer)
		{
			return init(renderer, _file);
		}
		...
		void addOutput(TiXmlElement* elemPass, RenderPass* pass, Renderer* renderer)
		{
			...
			GLMultiRenderTarget* target = new GLMultiRenderTarget(pass->getName(), _width, _height);
			while(elemTexture2d)
			{
				const char* name = elemTexture2d->Attribute("name");
